...
class MultimodalDataset_continual(Dataset):

    def __init__(self, o, data_dir, subset=0, split="train", comb=None, train_ratio=None, subsample=None):
        super(MultimodalDataset_continual, self).__init__()
    
        self.combs, self.s = o.combs, o.s
        self.train_ratio = o.train_ratio
        assert subset < len(self.combs) == len(self.s), "Inconsistent subset specifications!"
        self.subset = subset
        self.comb = self.combs[subset] if comb is None else comb
        if train_ratio is not None: self.train_ratio = train_ratio
...
class MultiDatasetSampler_continual(th.utils.data.sampler.Sampler):

    def __init__(self, o, dataset, batch_size=1, shuffle=True, max_dataset_size=10000, replay_strategy=None):
        self.dataset = dataset
        self.batch_size = batch_size
        if shuffle:
            self.Sampler = th.utils.data.sampler.RandomSampler
        ...
        self.r = o.replay_num
        self.c = o.current_num
        print('%d task(s) used as replay data, %d task(s) used as current training data' % (self.r, self.c))
