...
def concerto_train_ref_query_multimodality(ref_rna_tf_path: str, ref_protein_tf_path: str, query_rna_tf_path: str, query_protein_tf_path: str, weight_path: str, super_parameters=None):
    if not os.path.exists(weight_path):
            os.makedirs(weight_path)
    if super_parameters is None:
    ...
    query_target_list_RNA = []
    query_target_list_Protein = []
    for i in tf_list_2:
        query_target_list_RNA.append(os.path.join(query_rna_tf_path, i))
        query_target_list_Protein.append(os.path.join(query_protein_tf_path, i))
    ...
    for epoch in range(super_parameters['epoch_fineturn']):
        np.random.shuffle(query_target_list_RNA)
        for RNA_file, Protein_file in zip(query_target_list_RNA, query_target_list_Protein):
            print(RNA_file)
            query_db_RNA = create_classifier_dataset_multi([RNA_file],
                                                batch_size=super_parameters['batch_size'],
                                                is_training=False,
                                                data_augment=False,
                                                shuffle_size=10000,
                                                )
            query_db_Protein = create_classifier_dataset_multi([Protein_file],
                                                batch_size=super_parameters['batch_size'],
                                                is_training=False,
                                                data_augment=False,
                                                shuffle_size=10000,
            ...
            test_cls_accuracy.reset_states()
            step = 0
            for (query_features_RNA, query_values_RNA,
                source_batch_RNA, source_id_RNA), \
                (query_features_protein, query_values_protein,
                source_batch_Protein, source_id_Protein) \
                    in (zip(query_db_RNA, query_db_Protein)):
            ...
            # for step, (source_features, source_values, source_batch, source_id) in enumerate(train_db):
                # enumerate
                with tf.GradientTape() as tape:
                    z1 = encode_network([[query_features_RNA, query_features_protein],
                                         [query_values_RNA, query_values_protein]], training=True)
                    z2 = decode_network([query_values_RNA, query_values_protein], training=True)
                    ssl_loss = simclr_loss(z1, z2, temperature=0.1)
                    loss = ssl_loss
                    train_loss(loss)
...
def concerto_test_multimodal(model_path: str, RNA_tf_path: str, Protein_tf_path: str, n_cells_for_sample=None,super_parameters=None,
    ...
    for RNA_file, Protein_file in zip(train_source_list_RNA, train_source_list_Protein):
        ...
        RNA_id = []
        all_samples = 0
        # print('train_db_RNA:', len(train_db_RNA))
        for (source_features_RNA, source_values_RNA,
             source_batch_RNA, source_id_RNA), \
            (source_features_protein, source_values_protein,
             source_batch_Protein, source_id_Protein) \
